...
} from "./lock-configuration-sheet/lock-configuration-sheet.component";
...
export class LockFlagButtonComponent implements AfterViewInit {
  ...
  @ViewChild("tooltipParent", { static: false }) buttonElm: ElementRef;
  ...
  getLockedPropertyNames(): Array<string> {
    if (this.model && this.model.lockedProperties) {
      return this.propertyKeyList().filter(
        key => this.model.lockedProperties[key] === true
      );
    } else { return []; }
  }
  ...
  onClicked() {
    this._internalSubscriptions.add(
      this.bottomSheet
        .open(LockConfigurationSheetComponent, {
          data: {
            level: this.lockType,
            model: this.model,
            trackingPlanManager: this.trackingPlanManager
          } as ConfigurationSheetData,
