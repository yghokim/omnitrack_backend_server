...
import {
  ...
  CreateUserAccountDialogData
} from "./create-user-account-dialog/create-user-account-dialog.component";
...
export class ExperimentParticipantsComponent
  ...
  @ViewChild(MatSort, { static: false }) participantSort: MatSort;
          ...
          flatMap(expService =>
            expService.getExperiment().pipe(
              ...
              tap(result => {
                ...
                this.participantDataSource = new MatTableDataSource(
                  result.participants
                );

                this.participantColumns = ["alias", "username", "group"]
                  .concat(this.getDemographicKeys())
                  .concat([
                    "status",
                    "rangeStart",
                    "excludedDays",
              ...
              flatMap(result =>
                result.expService.updateLatestTimestampsOfParticipants()
              )
            )
          )
        )
  ...
  activeParticipantCount() {
    ...
    return this.participants.filter(
      participant => participant.participationInfo.dropped !== true
    ).length;
  }
    ...
    return this.api.selectedExperimentService.pipe(
      ...
      map(experiment => {
        const group = experiment.groups.find(
          g => g._id === participant.participationInfo.groupId
        );
        if (group) {
          return group.name;
        } else {
  ...
  getParticipantDemographicAnswer(
        ...
        .subscribe(ok => {
          if (ok === true) {
            this.api.selectedExperimentService
              .pipe(
                flatMap(expService =>
                  expService.removeParticipant(participantId)
                )
              )
              .subscribe(removed => {
                if (removed === true) {
                  this.notificationService.pushSnackBarMessage({
                    message: "Removed the participant entry."
                  });
                }
              });
          }
      ...
      this.dialog
        .open(YesNoDialogComponent, {
          data: {
            ...
            message:
              "Do you want to drop the participant from this experiment?",
            positiveLabel: "Drop",
            positiveColor: "warn",
            negativeColor: "primary"
          }
        ...
        .subscribe(ok => {
          if (ok === true) {
            this.api.selectedExperimentService
              .pipe(
                flatMap(expService => expService.dropParticipant(participantId))
              )
              .subscribe(removed => {
                if (removed === true) {
                  this.notificationService.pushSnackBarMessage({
                    message: "Dropped the participant."
                  });
                }
              });
          }
        ...
        .open(TextInputDialogComponent, {
          data: {
            ...
            validator: text => {
              return (
                (text || "").length > 0 &&
                text.trim() !== participant.participationInfo.alias
              );
            },
            submit: text =>
              this.api.selectedExperimentService.pipe(
                flatMap(service =>
                  service.changeParticipantAlias(participant._id, text.trim())
                ),
                catchError(err => {
                  switch (err.error) {
                    case "AliasAlreadyExists":
                      throw { error: "The same alias already exists." };
                    default:
                      throw err.error;
                  }
                })
  ...
  onChangeExperimentRangeStartInput(newDate: Date, participant) {
    this._internalSubscriptions.add(
      this.api.selectedExperimentService
        .pipe(
          flatMap(service =>
            service.updateParticipant(participant._id, {
              "experimentRange.from": newDate.getTime()
            })
          )
        )
        .subscribe(
          () => {
            this.notificationService.pushSnackBarMessage({
              message: "Modified the experiment start date of the participant."
            });
          },
          err => {
            console.log(err);
  ...
  onExcludedDaysEditClicked(participant: IUserDbEntity) {
    this._internalSubscriptions.add(
      this.dialog
        .open(ParticipantExcludedDaysConfigDialogComponent, {
          data: {
        ...
        .afterClosed()
        .pipe(
          flatMap((newDates: Array<Date>) => {
            if (newDates) {
              return this.api.selectedExperimentService.pipe(
                flatMap(expService =>
                  expService.setParticipantExcludedDays(
                    participant._id,
                    newDates
                  )
                )
  ...
  onExperimentAppSyncClicked(participant: any) {
    this._internalSubscriptions.add(
      this.api.selectedExperimentService
        .pipe(
          flatMap(expService =>
            expService.sendClientFullSyncMessages(participant._id)
          )
        )
        .subscribe(
          res => {
  ...
  onGeneratePasswordResetLinkClicked(participantId: string) {
    this._internalSubscriptions.add(
      this.api.selectedExperimentService
        .pipe(
          flatMap(expService =>
            expService.generateParticipantPasswordResetLink(participantId)
          )
        )
        .subscribe(link => {
  ...
  setSortParticipants(): void {
    ...
    this.participantDataSource.sortingDataAccessor = (
      data: any,
      sortHeaderId: string
    ) => {
      if (data) {
        switch (sortHeaderId) {
          case "alias": {
            return data.participationInfo.alias || "";
          }
          case "username": {
            if (data) {
          ...
          case "status": {
            if (data.participationInfo.dropped) {
              return 2;
            } else {
              return 1;
            }
          ...
          case "rangeStart": {
            if (data.participationInfo.experimentRange) {
              return data.participationInfo.experimentRange.from;
            }
            break;
          }
          case "joined": {
            return data.participationInfo.approvedAt || "";
          }
          case "created": {
            if (data) {
          ...
          case "lastSession":
            return data["lastSessionTimestamp"] || "";
          case "userId": {
            if (data) {
              return data._id || "";
            }
  ...
  onCreateNewUserClicked() {
    this._internalSubscriptions.add(
      this.dialog
        .open(CreateUserAccountDialogComponent, {
          data: {
