...
import {
  IResearchMessage,
  DefaultNewMessage,
  MessageReceiverRules,
  SpecificUsersMessageReceiverRule
} from "../../../../omnitrack/core/research/messaging";
...
export class ComposeMessageComponent implements OnInit, OnDestroy {
  ...
  private mountMessage(message: IResearchMessage) {
    ...
    switch (message.receiverRule.type) {
      case MessageReceiverRules.SpecificUsersRule:
        const userId = (message.receiverRule as SpecificUsersMessageReceiverRule)
          .userId;
  ...
  private enqueueMessage() {
    switch (this.mountedMessage.receiverRule.type) {
      case MessageReceiverRules.SpecificUsersRule:
        (this.mountedMessage
          .receiverRule as SpecificUsersMessageReceiverRule).userId = this.selectedParticipantUserIds;
        break;
    }
    ...
    this._internalSubscriptions.add(
      this.api.selectedExperimentService
        .pipe(flatMap(service => service.enqueueMessage(this.mountedMessage)))
        .subscribe(success => {
          console.log(success);
  ...
  public isDraftContentAvailable(): boolean {
    return (
      this.mountedMessage != null &&
      !this.isNullOrEmpty(this.mountedMessage.label) &&
      (!this.isNullOrEmpty(this.mountedMessage.messageBody) ||
        (this.mountedMessage.receiverRule.type ===
          MessageReceiverRules.SpecificUsersRule &&
          this.selectedParticipantUserIds.length > 0))
    );
  }
