...
export class ExperimentMessagingComponent implements OnDestroy, AfterViewInit {
  ...
  @ViewChild(MatSort, { static: false }) messageSort: MatSort;
  @ViewChild("draftTable", { read: MatSort, static: false }) draftSort: MatSort;
  ...
  ngAfterViewInit() {
    this._internalSubscriptions.add(
      this.api.selectedExperimentService
        .pipe(flatMap(service => service.getMessageList()))
        .subscribe(messages => {
          this.messageList = messages;
          this.messageDataSource = new MatTableDataSource(
            this.getMessageList()
          );
  ...
  makeReceiversText(message: IResearchMessage): string {
    if (!message.receivers || message.receivers.length == 0) {
      ...
      return (
        message.receivers[0].alias + " and " + (message.receivers.length - 1)
      );
    } else {
  ...
  setSorts(): void {
    this.messageDataSource.sortingDataAccessor = (
      data: IResearchMessage,
      sortHeaderId: string
    ) => {
      if (data) {
        if (sortHeaderId === "receivers") {
          ...
          if (data.receiverRule) {
            return data.receiverRule.type || "";
          }
        } else {
